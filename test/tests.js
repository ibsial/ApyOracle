...
describe("Float", function () {
    it("test initial value", async function () {
        ...
        //array needed for conversion testing only
        let numbersToPost = [];
        ...
        for (number in orderedApyArr) {
            ...
            //array needed for conversion testing only
            numbersToPost.push(
                finalNumber + "0".repeat(3 - decimalPart.length)
            );
        ...
        console.log("apy sent: ", numbersToPost);
        console.log("apy written: ", ApyWritten);

        let trueArray = [];
        for (let i = 0; i < numbersToPost.length; i++) {
            if (parseInt(numbersToPost[i]) == parseInt(ApyWritten[i])) {
                trueArray.push(true);
            } else {
